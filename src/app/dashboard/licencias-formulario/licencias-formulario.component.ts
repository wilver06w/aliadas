...
@Component({
  selector: 'app-licencias-formulario',
  templateUrl: './licencias-formulario.component.html',
  styleUrls: ['./licencias-formulario.component.css']
})
export class LicenciasFormularioComponent {

  public id: string | any;

  createForm: FormGroup;

  constructor(private activateRoute: ActivatedRoute, private LicenciasServices: LicenciasService) {
    ...
    this.createForm = new FormGroup({
      ...
      titulo_21: new FormControl('', []),
      texto_21: new FormControl('', []),
    })
  }
  ...
  ngOnInit() {
    this.activateRoute.params.subscribe(async params => {
      const id = params['id'];
      this.id = id;
      const miContenido = await this.LicenciasServices.getById(id);
  ...
  async onSubmit() {
    const results = await this.LicenciasServices.updateLicencias(this.id, this.createForm.value);
